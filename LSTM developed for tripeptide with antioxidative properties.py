...
# Load the data
X_new = pd.read_csv('C:/Users/RH/ACE_regression/develop_this/data/data_ABTS_320.csv', delimiter=',',header = None,index_col= 0, skiprows=1)
X = X_new.values
print(X.shape)
...
if __name__ == '__main__':
    ...
    ax.set_xlabel('Measured values($μmol\quadTE\cdotμmol\quadpeptide^{-1}$)', fontsize=7)
    ax.set_ylabel("Predicted values($μmol\quadTE\cdotμmol\quadpeptide^{-1}$)", fontsize=7)
    ...
    ax.set(xlim=(-0.5, 7), ylim=(-0.5, 7))
    plt.savefig("Figure 5.jpeg", bbox_inches='tight')
